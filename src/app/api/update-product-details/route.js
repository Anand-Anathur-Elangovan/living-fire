...
export async function PUT(req) {
  try {
    ...
    const { p_id, product_details, ptype_name } = body;
    if (!p_id || !product_details || !ptype_name) {
      return new Response(JSON.stringify({ error: "Missing required fields" }), { status: 400 });
    }

    // Update master table
    const masterQuery = `UPDATE tbl_master SET product_details = $2 WHERE p_id = $1 RETURNING *`;
    const masterValues = [p_id, JSON.stringify(product_details)];
    ...
    if (table) {
      const extraQuery = `UPDATE ${table} SET product_details = $2 WHERE p_id = $1 RETURNING *`;
