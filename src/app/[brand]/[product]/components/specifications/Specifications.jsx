...
const Specifications = ({ specifications, p_id, isAdmin }) => {
  ...
  useEffect(() => {
    if (specifications && !editMode) {
      setEditSpecs(JSON.parse(JSON.stringify(specifications)));
    }
  }, [specifications, editMode]);
  ...
  const parsedSpecifications = editMode ? editSpecs : specifications;
            ...
            >
              {parsedSpecifications?.map((spec, index) => (
                <motion.div 
                  ...
                  className={styles.specificationSection}
                  variants={itemVariants}
                  style={editMode ? {border: "1px solid #e0e0e0", borderRadius: "8px", padding: "16px", marginBottom: "16px"} : {}}
                >
                  {editMode ? (
                    <div style={{display: "flex", alignItems: "center", marginBottom: "12px", gap: "8px", flexWrap: "wrap"}}>
                      <input type="text" value={spec.spec_name} style={{fontWeight:"bold",fontSize:"1.1rem",border:"1px solid #ccc",borderRadius:4,padding:"8px 12px",flex:1,minWidth:"200px"}}
                        onChange={e=>{
                          const updated = [...editSpecs];
                          updated[index].spec_name = e.target.value;
                          setEditSpecs(updated);
                      ...
                      />
                      <button style={{padding:"6px 12px",fontSize:13,borderRadius:4,border:"1px solid #e00",background:"#fff",color:"#e00",cursor:"pointer",fontWeight:"bold"}} onClick={()=>{
                        const updated = editSpecs.filter((_,i)=>i!==index);
                        setEditSpecs(updated);
                      }}>Delete Header</button>
                      <button style={{padding:"6px 12px",fontSize:13,borderRadius:4,border:"1px solid #1741be",background:"#fff",color:"#1741be",cursor:"pointer"}} onClick={()=>{
                        const updated = [...editSpecs];
                        updated.splice(index+1,0,{spec_name:"New Header",spec_value:[]});
                        setEditSpecs(updated);
                      }}>Add Header</button>
                      <button style={{padding:"6px 12px",fontSize:13,borderRadius:4,border:"1px solid #28a745",background:"#fff",color:"#28a745",cursor:"pointer"}} onClick={()=>{
                        const updated = [...editSpecs];
                        updated[index].spec_value.push({name:"New Spec",value:""});
                        setEditSpecs(updated);
                      }}>Add Spec</button>
                  ...
                  <div className={styles.specItems}>
                    {spec.spec_value.map((item, idx) => {
                      if (editMode) {
                        ...
                        if (item.name === "Wood Fires" || item.name === "Gas Fires") {
                          return (
                            <div key={idx} style={{marginBottom:16,padding:"12px",border:"1px solid #eee",borderRadius:"6px"}}>
                              <div style={{display:"flex",alignItems:"center",marginBottom:"8px",gap:"8px"}}>
                                <input type="text" value={item.name} style={{fontWeight:"bold",border:"1px solid #ccc",borderRadius:4,padding:"6px 8px",flex:1}}
                                  onChange={e=>{
                                    const updated = [...editSpecs];
                                    updated[index].spec_value[idx].name = e.target.value;
                                    setEditSpecs(updated);
                                ...
                                />
                                <button style={{padding:"4px 8px",fontSize:12,borderRadius:4,border:"1px solid #e00",background:"#fff",color:"#e00",cursor:"pointer"}} onClick={()=>{
                                  const updated = [...editSpecs];
                                  updated[index].spec_value = updated[index].spec_value.filter((_,i)=>i!==idx);
                                  setEditSpecs(updated);
                                }}>Delete</button>
                                <button style={{padding:"4px 8px",fontSize:12,borderRadius:4,border:"1px solid #1741be",background:"#fff",color:"#1741be",cursor:"pointer"}} onClick={()=>{
                                  const updated = [...editSpecs];
                                  updated[index].spec_value.splice(idx+1,0,{name:"New Category",value:[]});
                                  setEditSpecs(updated);
                                }}>Add Category</button>
                              </div>
                              
                              {/* Nested value array */}
                              {item.value && item.value.map((energySpecItem, energySpecIndex) => (
                                <div key={energySpecIndex} style={{display:"flex",alignItems:"center",margin:"8px 0",gap:"8px",marginLeft:"16px"}}>
                                  <input type="text" value={energySpecItem.name} style={{border:"1px solid #ccc",borderRadius:4,padding:"4px 8px",width:"40%"}}
                                    onChange={e=>{
                                      const updated = [...editSpecs];
                                      updated[index].spec_value[idx].value[energySpecIndex].name = e.target.value;
                                      setEditSpecs(updated);
                                  ...
                                  />
                                  <input type="text" value={energySpecItem.value} style={{border:"1px solid #ccc",borderRadius:4,padding:"4px 8px",width:"40%"}}
                                    onChange={e=>{
                                      const updated = [...editSpecs];
                                      updated[index].spec_value[idx].value[energySpecIndex].value = e.target.value;
                                      setEditSpecs(updated);
                                    }}
                                  />
                                  <button style={{padding:"2px 6px",fontSize:11,borderRadius:4,border:"1px solid #e00",background:"#fff",color:"#e00",cursor:"pointer"}} onClick={()=>{
                                    const updated = [...editSpecs];
                                    updated[index].spec_value[idx].value = updated[index].spec_value[idx].value.filter((_,i)=>i!==energySpecIndex);
                                    setEditSpecs(updated);
                                  }}>Delete</button>
                                  <button style={{padding:"2px 6px",fontSize:11,borderRadius:4,border:"1px solid #1741be",background:"#fff",color:"#1741be",cursor:"pointer"}} onClick={()=>{
                                    const updated = [...editSpecs];
                                    updated[index].spec_value[idx].value.splice(energySpecIndex+1,0,{name:"New Field",value:""});
                                    setEditSpecs(updated);
                                  }}>Add</button>
                              ...
                              <button style={{marginLeft:"16px",padding:"4px 8px",fontSize:12,borderRadius:4,border:"1px solid #28a745",background:"#fff",color:"#28a745",cursor:"pointer"}} onClick={()=>{
                                const updated = [...editSpecs];
                                if (!updated[index].spec_value[idx].value) {
                                  updated[index].spec_value[idx].value = [];
                                }
                                updated[index].spec_value[idx].value.push({name:"New Field",value:""});
                                setEditSpecs(updated);
                          ...
                          return (
                            <div key={idx} style={{display:"flex",alignItems:"center",marginBottom:"8px",gap:"8px",padding:"8px",border:"1px solid #f0f0f0",borderRadius:"4px"}}>
                              <input type="text" value={item.name} style={{border:"1px solid #ccc",borderRadius:4,padding:"4px 8px",flex:1}}
                                onChange={e=>{
                                  const updated = [...editSpecs];
                                  updated[index].spec_value[idx].name = e.target.value;
                                  setEditSpecs(updated);
                              ...
                              />
                              <input type="text" value={item.value} style={{border:"1px solid #ccc",borderRadius:4,padding:"4px 8px",width:"120px"}}
                                onChange={e=>{
                                  const updated = [...editSpecs];
                                  updated[index].spec_value[idx].value = e.target.value;
                                  setEditSpecs(updated);
              ...
              {error && <div style={{color:"red",marginTop:8,padding:8,backgroundColor:"#ffe6e6",borderRadius:4}}>{error}</div>}
              {success && <div style={{color:"green",marginTop:8,padding:8,backgroundColor:"#e6ffe6",borderRadius:4}}>{success}</div>}
              {loading && <div style={{color:"#666",marginTop:8,padding:8,backgroundColor:"#f0f0f0",borderRadius:4}}>Updating...</div>}
            </motion.div>
          </motion.div>
        </div>
      </motion.div>
